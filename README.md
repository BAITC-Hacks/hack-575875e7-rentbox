...
[Проверка](#проверка-за-две-минуты) · [Ограничения](#известные-ограничения)
...
- **Прогноз в 2.15 раза точнее базовой планки:** MAE 0.15947 против 0.34268
  у наивного прогноза «как в последний известный час», на том же горизонте
  24–48 часов.
- **Найдено 226 МВт·ч в год недовыработки**, о которой в постановке задачи не
  говорилось: обе турбины простаивают при рабочем ветре 6–12 м/с — первая 2.2%
  времени, вторая 1.5%. При номинале 2.5 МВт и тарифе 25–30 ₸/кВт·ч это
  **5.7–8.1 млн ₸ в год**. Разбор — в [бэклоге](docs/BACKLOG.md).
- **Видно, какая турбина стоит.** Площадки в 300 метрах видят один ветер, поэтому
  отношение их выработки не зависит от погоды: в июне 2024 первая простаивала
  22.2% времени при нулевом простое второй, в феврале 2025 — наоборот.
...
**Что получится** — `artifacts/demo-forecast.csv`, 96 строк: 48 часов × 2 турбины.
...
1,2026-01-31T21:00:00Z,3,0.02038670765856902
1,2026-01-31T22:00:00Z,4,0.03574282117187977
```

В полном файле у каждой строки есть ещё версия модели и происхождение погоды:
какой цикл NOAA использован, когда он был опубликован и SHA-256 исходного объекта.
...
**Дашборд пока показывает синтетические данные** — подключение к API не завершено,
см. [ограничения](#известные-ограничения). Реальные результаты модели доступны
через `./run.sh demo`, CLI, API и CSV в репозитории.
...
Мы это не предполагаем, а **доказываем для каждого использованного файла**:
...
- сохранены URL, ETag, диапазоны байтов и SHA-256 каждого объекта — **730 циклов**
  с 01.03.2024 по 28.02.2026 лежат в репозитории;
...
**Модель в 2.15 раза точнее базовой планки** — 0.15947 против 0.34268. Это и есть
практический смысл работы: на горизонте суток ошибка более чем вдвое меньше, чем
у простого переноса последнего известного значения.

MAE 0.15947 означает среднее абсолютное отклонение 0.15947 по шкале 0–1: примерно
16% номинальной мощности. Оценка включает 2928 прогнозных строк; один целевой час
может встречаться в нескольких ежедневных выпусках. Ошибка ожидаемо растёт
с горизонтом: 0.15226 на сутки вперёд против 0.16692 на вторые.
...
Что должен увидеть проверяющий и за какое время.

```bash
./run.sh check                     # 5 секунд: окружение, данные, кэш погоды
./run.sh demo                      # 2 минуты: сборка образа и расчёт прогноза
```
...
|---|---|
| `check` | четыре строки `✓`: Docker, оба CSV, кэш погоды |
| `demo` | `✓ получено 96 почасовых значений → artifacts/demo-forecast.csv` |
| файл | 97 строк: заголовок и 48 часов × 2 турбины |
| метрики | `artifacts/gfs-model/metrics.json` содержит январский MAE `0.15947` — то же число, что в таблице выше |
| воспроизведение февраля | `artifacts/gfs-model/february_replay.csv`, 2784 строки, 29 ежедневных выпусков |
...
Если `.env` уже существует и получена ошибка `CONFIGURATION_REQUIRED` — не заданы
первые четыре настройки. Быстрее всего исправить через `make demo` либо удалить
`.env` и запустить `./run.sh`, который создаст его заново.
...
Распределение ниже соответствует истории репозитория и проверяется командой:

```bash
git log --format='%an' --numstat | awk 'NF==1{a=$0} NF==3{f[a]++} END{for(k in f) print k, f[k]}'
git log --author=<ник> --name-only --format=''
```

| Участник | Учётная запись | Зона ответственности | Основные каталоги |
|---|---|---|---|
| `<ФИО>` (капитан) | `flyperry` | Данные и аудит, погодный модуль NOAA, обучение и отбор моделей, агент, хранилище прогнозов, запуск и документация | `src/`, `scripts/`, `artifacts/`, `data/`, `reports/` |
| `<ФИО>` | `Romario` | HTTP API: маршруты, схемы, сервисы, очередь запусков, контракт и OpenAPI | `backend/app/`, `docs/API.md` |
| `<ФИО>` | `Carcadju` | Дашборд Windcast: интерфейс, библиотека компонентов, 3D-визуализация турбины | `apps/web/`, `packages/ui/` |

> Перед сдачей подставить ФИО вместо `<ФИО>`.
