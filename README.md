...
Версия дизайна v2 находится в ветке `design/dashboard-v2`: KPI над графиком, компактная 3D-панель, единая шкала времени и кликабельные выводы. Язык выбирается в шапке и сохраняется в браузере; переключение не сбрасывает прогноз. Там же доступны тёмная тема и версия для слабовидящих: крупный текст, высокий контраст и статичная турбина. Режимы можно сочетать, настройки сохраняются. Предыдущая версия сохранена тегом `dashboard-before-v2`; инструкция по возврату — в [документации frontend](docs/FRONTEND.md#dashboard-v2-and-languages).
