...
**В дашборде два режима.** Основной — реальный прогноз через API. Второй —
**синтетическая симуляция выработки**: погодные сценарии, обледенение и почасовое
воспроизведение на сгенерированных данных (`apps/web/lib/wind-simulation.ts`,
тесты в `tests/wind-simulation.test.mjs`). Симуляция нужна для демонстрации
интерфейса и не связана с моделью: её числа **не являются прогнозом** и помечены
в интерфейсе как «Симуляция выработки».
